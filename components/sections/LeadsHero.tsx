...
export default function LeadsHero() {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_460px] gap-6 lg:gap-9 items-start py-8 lg:py-16">
        
        {/* Hero Left Content */}
        <div
          className="p-4 lg:p-6 order-2 lg:order-1 rounded-[18px]"
          style={{
        ...
        >
          ...
          >
            {currentData.kpis.map((kpi, index) => (
              <div
                key={index}
                className="min-w-[120px] sm:min-w-[130px] flex-1 sm:flex-none text-center sm:text-left p-3 rounded-xl"
                style={{
        ...
        >
          {/* Main Image Card */}
          <div
            className="w-full max-w-md lg:max-w-none p-4 lg:p-[18px] rounded-[14px]"
            style={{
          ...
          >
            <img
              className="w-full block rounded-xl h-[200px] sm:h-[240px] lg:h-[260px] object-cover object-center"
            ...
            {currentData.showCardFooter && currentData.cardFooter && (
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-3 gap-3 sm:gap-0 p-5">
                ...
                <div>
                  <button
                    onClick={() => scrollToSection('#contact')}
                    className="w-full sm:w-auto px-3 py-2 text-sm lg:text-base rounded-[10px]"
                    style={{
          ...
          {currentData.showBottomCards && currentData.bottomFeatures && (
            <div className="flex flex-col sm:flex-row gap-2.5 mt-2.5 w-full max-w-md lg:max-w-none">
              {currentData.bottomFeatures.map((feature, index) => (
                <div
                  key={index}
                  className="flex-1 flex items-center gap-3 p-3 lg:p-[18px] rounded-[14px]"
                  style={{
                    background: 'linear-gradient(180deg,#fff,#fbfbff)',
                    boxShadow: '0 6px 20px rgba(16,24,40,0.08)'
                  }}
                >
                  <img
                    ...
                    className="w-12 h-12 lg:w-16 lg:h-16 object-cover flex-shrink-0 rounded-lg"
                  />
                  <div>
                    <strong className="block text-sm lg:text-base" style={{ color: '#062235' }}>
                      {feature.title}
